...
const getTotalOfTheMonth = async (
  db: PlanetScaleDatabase<DBSchema>,
  userId: string,
  month: number,
  year: number,
) => {
...
export const dashboardRouter = createTRPCRouter({
  total: protectedProcedure
    .input(
      z.object({ month: z.number(), year: z.number() }).transform((input) => ({
        ...input,
        monthYear: `${input.month}/${input.year}`,
      })),
    )
    .query(async ({ ctx, input }) => {
      const currentMonth = await getTotalOfTheMonth(
        ctx.db,
        ctx.user.id,
        input.month,
        input.year,
      ...
      const previousMonth = await getTotalOfTheMonth(
        ctx.db,
        ctx.user.id,
        previous.month,
        previous.year,
      ...
      const percentage =
        totalPreviousMonth === 0 ? 0 : (diff / totalPreviousMonth) * 100;
